...
def train(hyp, opt, device, tb_writer=None):
    ...
    if opt.adam:
        optimizer = optim.Adam(
            pg0, lr=hyp['lr0'], betas=(hyp['momentum'],
    ...
    results = (0, 0, 0, 0, 0, 0,
               0)  # P, R, mAP@.5, mAP@.5-.95, val_loss(box, obj, cls)
        ...
        if rank in [-1, 0]:
            ...
            if (not opt.nosave) or (final_epoch and not opt.evolve):  # if save
                ckpt = {
                    ...
                    'model':
                    deepcopy(model.module
                             if is_parallel(model) else model).half(),
                    'ema':
...
if __name__ == '__main__':
    ...
    # Evolve hyperparameters (optional)
        ...
        meta = {
            'lr0': (1, 1e-5,
                    1e-1),  # initial learning rate (SGD=1E-2, Adam=1E-3)
            'lrf': (1, 0.01,
                    1.0),  # final OneCycleLR learning rate (lr0 * lrf)
