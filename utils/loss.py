...
class SigmoidBin(nn.Module):
    ...
    def forward(self, pred):
        ...
        pred_reg = (
            pred[..., 0] * self.reg_scale - self.reg_scale / 2.0) * self.step
...
class ComputeLoss:
    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if self.autobalance:
                self.balance[
                    i] = self.balance[i] * 0.9999 + 0.0001 / obji.detach(
                    ).item()
        ...
        for batch_idx in range(p[0].shape[0]):
            ...
            for i, pi in enumerate(p):
                ...
                pw = self.wh_bin_sigmoid.forward(
                    fg_pred[..., 2:(3 + self.bin_count)]
                    .sigmoid()) * anch[i][idx][:, 0] * self.stride[i]
                ph = self.wh_bin_sigmoid.forward(
                    fg_pred[..., (3 + self.bin_count):obj_idx]
                    .sigmoid()) * anch[i][idx][:, 1] * self.stride[i]

                pxywh = torch.cat(
                    [pxy, pw.unsqueeze(1),
                     ph.unsqueeze(1)], dim=-1)
                pxyxy = xywh2xyxy(pxywh)
                pxyxys.append(pxyxy)
...
class ComputeLossAuxOTA:
    ...
    def __call__(self, p, targets, imgs):  # predictions, targets, model
        ...
        for i in range(self.nl):  # layer index, layer predictions
            ...
            if n_aux:
                ...
                selected_tbox_aux = targets_aux[i][:, 2:
                                                   6] * pre_gen_gains_aux[i]
                selected_tbox_aux[:, :2] -= grid_aux
                iou_aux = bbox_iou(
                    pbox_aux.T, selected_tbox_aux, x1y1x2y2=False,
                    CIoU=True)  # iou(prediction, target)
    ...
    def build_targets(self, p, targets, imgs):
        ...
        for batch_idx in range(p[0].shape[0]):
            ...
            top_k, _ = torch.topk(
                pair_wise_iou, min(20, pair_wise_iou.shape[1]), dim=1)
            dynamic_ks = torch.clamp(top_k.sum(1).int(), min=1)
